<template>
    <div class="test">
        <div style="margin: 10px;padding-left: 10px">
            ...
            <el-button type="primary" @click.native="buildIdentity()" size="small">生成银行信息三要素</el-button>
            <el-button type="primary" @click.native="getSmscode()" size="small">获取短信验证码</el-button>
        ...
        <div>
          <template v-if="showBuildIdentityTable">
            <el-table
            :data="identityData"
            style="width: 100%">
            <el-table-column
        ...
        <template v-if="showSmscodeTable">
            <el-table
            :data="smscodeData"
            style="width: 100%">
            <el-table-column
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                showBuildIdentityTable: false,
                identityData: [],
                showSmscodeTable: false,
                smscodeData: [],
                data: [{
                    id: 1,
                    label: '一级 1',
            ...
            this.$axios.get('/api/buildIdentity', {}).then((response) => {
                    if (response.data['status'] === 0) {
                        ...
                        this.identityData = [{
                            ...
                            identity: response.data['data']['identity_data'],
                            bankcard: response.data['data']['bankcard'],
                            }];
                        this.showBuildIdentityTable = true;
                        this.showSmscodeTable = false;
                        // }
                    }
                }
            )
        ...
        getSmscode() {
            // 调用 callback 返回建议列表的数据
            this.$axios.get('/api/getSmscode', {}).then((response) => {
                    if (response.data['status'] === 0) {
                        this.$message({
                            showClose: true,
                            message: response.data['msg'],
                        ...
                        this.smscodeData = response.data['data'];
                        // this.smscodeData = [{mobile: '110', smscode: 'akjsdhasdn'},{mobile: '1101', smscode: 'akjsd1hasdn'}];
                        this.showSmscodeTable = true;
                        this.showBuildIdentityTable = false;
